...
INTERFACE="${1:-eth0}"
TARGET_CORE="${2:-2}" # Default to Ingress Core 2

log_info() { echo -e "\e[32m[IRQ]\e[0m $1"; }
log_err()  { echo -e "\e[31m[ERROR]\e[0m $1"; exit 1; }

[[ $EUID -ne 0 ]] && log_err "Root privileges required."
[[ ! -d "/sys/class/net/$INTERFACE" ]] && log_err "Interface $INTERFACE not found."

# 1. Determine Hex Mask for target core
MASK=$(printf "%x" $((1 << TARGET_CORE)))

# 2. Identify MSI-X Vectors for the interface
NIC_IRQS=$(grep "$INTERFACE" /proc/interrupts | awk '{print $1}' | tr -d ':' || true)

if [[ -z "$NIC_IRQS" ]]; then
    log_err "No IRQs found for $INTERFACE. Is the driver loaded?"
fi
...
# 4. Apply Hard Affinity
log_info "Locking $INTERFACE IRQs [$NIC_IRQS] to Core $TARGET_CORE (Mask: 0x$MASK)"
...
log_info "Interrupt affinity hardened."
