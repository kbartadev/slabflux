...
# See accompanying LICENSE and NOTICE files for the integrated terms of use.
# ============================================================================
# Hardware Ingress Filtering
# Only allow specific UDP traffic to reach our Core (Queue 2)

INTERFACE="eth0"
DEST_IP="10.0.0.5"
DEST_PORT="9000"

echo "[NIC] Configuring Flow Director for $INTERFACE..."

# 1. Clear existing rules
ethtool -N $INTERFACE delete-ntuple 0 2>/dev/null

# 2. Add hardware rule: Match IP/Port -> Redirect to Queue 2 (Our Queue)
ethtool -N $INTERFACE flow-type udp4 dst-ip $DEST_IP dst-port $DEST_PORT action 2

# 3. Drop all other traffic on this specific hardware queue
ethtool -N $INTERFACE flow-type udp4 action -1 # Default drop for noise
