...
# See accompanying LICENSE and NOTICE files for the integrated terms of use.
# ============================================================================
# PCIe ASPM & Latency Hardening
set -euo pipefail

PCI_ADDR="${1:-0000:01:00.0}" # Targeted device (e.g., Ethernet Controller)
log_info() { echo -e "\e[32m[PCIe]\e[0m $1"; }
log_err()  { echo -e "\e[31m[ERROR]\e[0m $1"; exit 1; }

[[ $EUID -ne 0 ]] && log_err "Root privileges required."
if ! command -v setpci &> /dev/null; then log_err "pciutils (setpci) missing."; fi

# 1. Global ASPM Suppression
# Disables OS-level power management for all PCI buses.
for policy in /sys/class/pci_bus/*/device/aspm_policy; do
    if [[ -f "$policy" ]]; then
        echo performance > "$policy" 2>/dev/null || true
    fi
done
log_info "Global PCIe ASPM policy set to performance."

# 2. Device-Specific L0 Lock
# Force the targeted high-speed device into L0 state (permanently active).
if lspci -s "$PCI_ADDR" &> /dev/null; then
    setpci -s "$PCI_ADDR" CAP_EXP+10.b=0
    log_info "Device $PCI_ADDR locked in L0 state."
else
    log_err "Device $PCI_ADDR not found on PCI bus."
fi

log_info "PCIe bus latency hardening complete."
