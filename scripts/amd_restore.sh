...
# See accompanying LICENSE and NOTICE files for the integrated terms of use.
# ============================================================================
# SlabFlux Benchmark Environment Restore Script
#
# This script re‑enables AMD Boost and refreshes the SMU sensor state.
# It is intended for deterministic benchmarking environments.
# Requires root privileges.
#!/usr/bin/env bash

echo "=== Restoring normal CPU behavior ==="

# 1) Re-enable SMT
echo "Re-enabling SMT..."
echo on | sudo tee /sys/devices/system/cpu/smt/control > /dev/null

# 2) Re-enable all C-states
echo "Re-enabling C-states..."
for f in /sys/devices/system/cpu/cpu*/cpuidle/state*/disable; do
    echo 0 | sudo tee $f > /dev/null
done

# 3) Restore governor to schedutil (default on modern Linux)
echo "Restoring CPU governor..."
sudo cpupower frequency-set -g schedutil > /dev/null

# 4) Remove frequency lock (set min to 0, max to unlimited)
echo "Removing frequency lock..."
sudo cpupower frequency-set -d 0 -u 9999999 > /dev/null

# 5) Re-enable AMD Boost
echo "Re-enabling AMD boost..."
echo 1 | sudo tee /sys/devices/system/cpu/cpufreq/boost > /dev/null

# 6) Reload SMU sensors (forces partial SMU state refresh)
echo "Refreshing SMU state..."
