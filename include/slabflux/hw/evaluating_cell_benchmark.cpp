...
protected:
    ...
    void PinThreadToCore(std::thread& th, int core_id) {
        ...
        if (rc != 0) {
            std::cerr << "[ WARNING ] Failed to set thread affinity to core " << core_id << "\n";
        }
#endif
    }
};

TEST_F(EvaluatingCellBenchmark, MeasureThroughputIsolated) {
    ...
    // In the optimal test environment, the cell is prioritized for a separate physical core
    PinThreadToCore(alu_thread, 2);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        if (current_phase == 0 || current_phase == PHASE_KILL) [[unlikely]] current_phase = 1;
    }
    ...
    // Cleanly stop the ALU thread by issuing a KILL Phase before reading performance
