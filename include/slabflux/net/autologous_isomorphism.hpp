...
namespace slabflux::net {
    ...
    public:
         ...
         * @return Pair containing the decoupled type_id (0 if void) and the payload.
         */
        SLAB_FORCE_INLINE std::pair<uint32_t, T> extract_and_decouple(uint32_t sequence_clock) const noexcept {
            ...
            uint32_t extracted_type = type_id_ & (validity_mask == 0xFFFF ? 0xFFFFFFFF : 0x00000000);
            
            // Note: The caller uses `extracted_type` to route the event. 
            // If extracted_type == 0, it routes to nowhere.
            return {extracted_type, output};
        }
    };
}
