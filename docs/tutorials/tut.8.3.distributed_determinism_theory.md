...
If a UDP multicast packet containing market data drops on Node A but arrives on Node B, Node A's pipeline will diverge. The `context_vault` state must remain identical across the mesh.
...
struct EpochArbiter {
    ...
    // The 'epoch' vault is resolved via SFINAE signature match
    bool on(SequencedEvent& e, EpochVault& epoch) {
