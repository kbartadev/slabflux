...
## 3. The Demuxer Implementation

The `demuxer` is a pure, static compile-time construct. It uses C++17 fold expressions to expand variadic type packs into a flat sequence of ID comparisons.

```cpp
namespace slabflux::core {
    
    template <typename... SupportedEvents>
    struct demuxer {
        
        template <typename PipelineType>
        static inline void route(tagged_pointer tp, PipelineType& pipe) noexcept {
            uint16_t current_tag = tp.tag();

            // Branchless dispatch via fold expression
            bool dispatched = ((SupportedEvents::ID == current_tag ? (
                pipe.dispatch(*reinterpret_cast<SupportedEvents*>(tp.ptr())), true
            ) : false) || ...);

#ifndef NDEBUG
            if (!dispatched) {
                std::cout << "[ERR] Unknown tag: " << current_tag << "\n";
            }
#endif
        }
    };
...
## 3.1 Unrolling and Branchless Logic
When you instantiate `demuxer<limit_order, market_tick>`, the compiler unrolls the fold expression. Because `ID` is `static constexpr`, the compiler resolves all potential tag matches at compile-time, often synthesizing a jump table or a sequence of `CMOV` (conditional move) instructions.
...
This example demonstrates a risk-validation gateway that uses the `demuxer` to steer untyped memory packets into specialized `pipeline` strategies.
...
int main() {
    ...
    slabflux::core::pipeline hft_gateway(risk_layer);
