...
## 2. The Inverse Priority Poset (`meta.hpp`)
...
We fix this by using `priority` tags from `slabflux::core::meta.hpp`.
...
int main() {
    slabflux::core::pipeline<DiamondHandler> pipe(DiamondHandler{});
    EventD leaf_event;
