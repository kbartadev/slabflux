 ...
 * ============================================================================*
 * @file rio_egress_test.cpp
 * @brief Windows Registered I/O (RIO) Egress Nexus Stress Test Suite.
 */
    ...
    public:
        explicit windows_block_allocator() noexcept {
            ...
            for (std::size_t i = 0; i < BlockCount; ++i) {
                ...
                std::memset(storage_[i].frame.data, 0x41, 1460);
            }
        }
...
// ============================================================================
// METRIC 1: MICROARCHITECTURAL RESIDENCY AUDIT (STRIDE VERIFIED)
// ============================================================================
TEST(RioEgressStressTest, MicroarchitecturalResidencyAudit) {
    using OutboundConduit = core::conduit;
    using MemoryPool      = windows_block_allocator<transport::raw_tcp_frame, 1024>;
    ...
    try {
        transport::rio_egress<OutboundConduit> egress_engine(
        ...
        FAIL() << "Failed to construct Registered IO Egress memory structures.";
    }
...
// ============================================================================
// METRIC 2: SATURATION FLUSHING PHYSICS
// ============================================================================
TEST(RioEgressStressTest, SaturationThroughputPhysics) {
    using OutboundConduit = core::conduit;
    using MemoryPool      = windows_block_allocator<transport::raw_tcp_frame, 4096>;
    ...
    RIO_EXTENSION_FUNCTION_TABLE rio_table;
    ASSERT_TRUE(load_rio_extension_table(tx_client_socket, rio_table));

    transport::rio_egress<OutboundConduit> egress_engine(
        tx_client_socket, rio_table, out_conduit, running
    );

    constexpr std::size_t BENCHMARK_ITERATIONS = 50'000;

    for (std::size_t i = 0; i < 1024; ++i) {
        auto* frame = mem_pool.make_raw();
        if (frame) {
            frame->payload_length = 512;
            frame->connection_id  = static_cast<uint32_t>(tx_client_socket);
    ...
    for (std::size_t i = 0; i < BENCHMARK_ITERATIONS; ++i) {
        egress_engine.poll_egress(rio_table);

        if (__builtin_expect(i % 4 == 0, 0)) {
            while (recv(rx_server_socket, drain_buffer, sizeof(drain_buffer), 0) > 0) {}
        }

        if (__builtin_expect(out_conduit.occupancy() < 512, 0)) {
            auto* frame = mem_pool.make_raw();
            if (frame) {
                frame->payload_length = 512;
                frame->connection_id  = static_cast<uint32_t>(tx_client_socket);
    ...
    std::cout << "[PERF] Windows RIO Egress Validated Throughput: " << millions_of_ops_per_sec << " Mops/sec\n";
    std::cout << "[PERF] Windows RIO Egress Real Loop Latency: " << precision_cycles_per_poll << " cycles/poll\n";

    EXPECT_LT(precision_cycles_per_poll, 45.0);
    EXPECT_GT(millions_of_ops_per_sec, 1.0);

    closesocket(tx_client_socket);
    closesocket(rx_server_socket);
    WSACleanup();
}

// ============================================================================
// FIXED METRIC 3: TRUE POOL EXHAUSTION HARDWARE BACKPRESSURE INTEGRITY
// ============================================================================
TEST(RioEgressStressTest, BackpressureRingSaturationStability) {
    using OutboundConduit = core::conduit;
    using MemoryPool      = windows_block_allocator<transport::raw_tcp_frame, 64>;
    ...
    // 1. Flood the conduit completely up to the allocator's max pool limits
    // Before ticking the engine once. This ensures the queue is packed.
    ...
    // 2. Drive the engine loops repeatedly against the frozen TCP window.
    // The first poll drains 32 elements (MAX_SLOTS) into RIO and fills the queue.
    // The remaining poll ticks verify that the engine safely bounces or blocks
    // when it loops over an empty conduit while the allocator pool is completely dry.
    for (std::size_t i = 0; i < 64; ++i) {
        egress_engine.poll_egress(rio_table);
    }

    SUCCEED() << "Windows RIO Egress successfully verified under combined pool and descriptor exhaustion bounds.";
